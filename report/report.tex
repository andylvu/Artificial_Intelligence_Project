...
\IEEEtitleabstractindextext{%
    \begin{abstract}
        This paper is to present the current progress of our optimal grocery list
        generator. The nature of our problem causes it to identify as a
        multidimensional knapsack problem. With this in mind we have devised
        approaches to solve the knapsack problem. We have developed a custom dataset
        that allows easily manipulated able data values that coincide with our
        specific needs. On top of this we have developed two solutions: a brute
        force approach as well as a dynamic programming approach. With our initial
        preliminary testing, results show that our brute force approach is showing
        typical performance of a brute force method having difficulty once the
        dataset becomes of an impractical size. The dynamic programming approach
        shows a more promising result when comparing time complexity with it able to
        complete calculations with the entirety of our dataset. With these baselines
        covered, our future goals is to implement a genetic algorithm that can
        processes the problem much more easily combined with our user interface that
        can handle the problem as a whole including food group considerations.
    \end{abstract}
...
\begin{table}[h]
    ...
    \centering
    \include{assets/example_groceries.tex}
...
\begin{table}[h]
    \setlength\tabcolsep{2pt}
    \centering
    \include{assets/bruteforce_results.tex}
...
\begin{table}[h]
    \setlength\tabcolsep{2pt}
    \centering
    \include{assets/dynamic_runtime.tex}
